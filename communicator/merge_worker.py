...
def merge_two_checkpoints(ckpt1_data, ckpt2_data, compressor_ratio, device_id):
    ...
    with torch.cuda.device(device_id):
        for key in ckpt1_data.keys():
            # 1. Decompress both checkpoints to full tensors (on GPU)
            ...
            # 2. Merge tensors (on GPU)
            merged_tensor = tensor1 + tensor2
            
            # 3. Re-compress using Top-K (on GPU)
            ...
            # 4. Move result back to CPU to avoid GPU memory accumulation
            merged_data[key] = {
                'values': values.cpu(),
                'indices': indices.cpu(),
...
def queue_worker(task_queue, result_queue, device_id):
    ...
    This function runs in a separate process and communicates via multiprocessing.Queue.
    It avoids importing parallel_recovery.py, thus preventing loading of heavy modules.
    ...
    """
    import os
    # Set environment variables to prevent DeepSpeed auto-initialization
